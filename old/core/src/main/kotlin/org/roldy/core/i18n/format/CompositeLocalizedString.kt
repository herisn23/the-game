...
class CompositeLocalizedString(
    ...
    val arguments: Map<String, Selector> by lazy {
        strRegexp.findAll(str).associate { result ->
    ...
    private fun String.resolveSelector(): Selector =
        split(":").let { selectors ->
            when (selectors.size) {
                ...
                //when only one selector appear e.g. {color} then is resolved as argument
                1 -> listOf(Selector.Type.Argument.code, selectors.first())
                else -> selectors
            }
        }.let { selectors ->
    ...
    fun resolve(arguments: Map<String, String?>): String =
