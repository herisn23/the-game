...
class FoliagePopulator(
    ...
    foliage: List<FoliageTileData>,
    val tilesAtlas: TextureAtlas,
) : AutoDisposableAdapter(), WorldChunkPopulator {
    val mountains = foliage.associateBy { it.coords }
    ...
    override fun populate(
        ...
        return data.mapNotNull { (coords, _) ->
            mountains[coords]?.let { foliage ->
                SpriteTileBehaviour.Data(
                    layer = Layered.LAYER_2,
                    position = worldPosition(coords),
                    coords = coords,
                    textureRegion = tilesAtlas[foliage.data.name],
                    data = mapOf("data" to foliage)
                )
            }
        }
    }
