...
import org.roldy.rendering.g2d.gui.el.autoupdate
...
class HarvestingWindow(
    ...
    private val minedLabel: LabelActions,
    ...
    var mineState: MineState? by Delegates.observable(null) { property, oldValue, newValue ->
        if (newValue != null) {
            val mineLocKey = newValue.harvestable.mineType.locKey
            remainingLabel.setText(translate { mine_supplies.arg("amount", newValue.refreshing.current) })
            minedLabel.setText(translate { harvesting_amount[mineLocKey].arg("amount", 0) })
            ...
            minedLabel.setText(null)
            harvestableLabel.setText(null)
            uiWindow.title.setText(null)
            harvestButton.setText(null)
        }
    }
...
fun <S> UIWidget<S>.harvestingWindow(init: (HarvestingWindow) -> Unit): WindowActions {
    ...
    lateinit var mined: LabelActions
    ...
    init(HarvestingWindow(window, remaining, mined, harvestable, mine, collect))
    return window
}