...
class MineGenerator(
    ...
    private fun generateMinesInSettlements(): MutableList<MineTileData> {
        val mines = mutableListOf<MineTileData>()
        settlements().forEach { settlement ->
            val radius = settlement.claims.toMutableSet()
            val settlementMines = mutableListOf<MineTileData>()
            repeat(settlement.harvestableCount) { i ->
                ...
                val harvestable =
                    pickHarvestableByBiome(random, minePosition, settlementMines.map(MineTileData::harvestable))
                MineTileData(
                    coords = minePosition,
    ...
    private fun generateMinesOutsideSettlements(): MutableList<MineTileData> {
        val mines = mutableListOf<MineTileData>()
